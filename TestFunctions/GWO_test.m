...
for funcIdx = 1:length(testFunctions)
    ...
    for run = 1:numRuns
        tic;
         [~, bestFitness, fitness_iter] = GWO_function(N, maxIter, aMax, minError, testFunction, range, dim);
        runTimes(run) = toc;
...
% 灰狼优化算法主函数
function [best_solution, best_fitness, fitness_iter] = GWO_function(N, maxIter, aMax, minError, testFunction, range, dim)
    lb = range(1); % 下界
    ub = range(2); % 上界
