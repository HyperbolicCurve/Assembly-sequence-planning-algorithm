...
maxGenerations = 300; % 最大代数
crossoverRate = 0.8; % 交叉概率
mutationRate = 0.1; % 变异概率
minError = 1e-2; % 算法终止的最小误差
...
% 计算最优解的具体指标
[N_g, N_t, N_d, N_s] = calculate_indicators(bestSolution.sequence, S, T, C, bestSolution.directions, A_I_X, A_I_Y, A_I_Z);

fprintf('最优装配序列: ');
disp(bestSolution.sequence);
fprintf('最优适应度: %f\n', best_fitness);
fprintf('装配工具的改变次数: %d\n', N_t);
fprintf('装配方向的改变次数: %d\n', N_d);
fprintf('装配过程中不稳定操作的次数: %d\n', N_s);
fprintf('零件的安装方向: ');
disp(bestSolution.directions);

% 绘制目标函数值和代数的关系图
figure;
plot(1:generation, bestFitnessValues, '-o');
xlabel('代数');
ylabel('目标函数值');
title('目标函数值与代数的关系');
